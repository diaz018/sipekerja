...
class Sipekerja_Kegiatan_model extends CI_model{

    public function setKegiatan(Array $data)
   {
      $this->db->insert('sipekerja_kegiatan', $data);
   }

    public function getKegiatan($jenis_kegiatan = "",$jenis_sampel = "", $jenis_bidang ="" ){
        // $query = $this->db->get('sipekerja_kegiatan');
        // return $query->result_array();
        
        $this->db->select('*');
        $this->db->from('sipekerja_kegiatan');
    ...
    public function deleteKegiatanById($id ){
        $this -> db -> where('id_kegiatan', $id);
        $this -> db -> delete('sipekerja_kegiatan');
    }
    ...
    public function ubahStatusKegiatanById($id, $status){
        ...
        $this->db->where('id_kegiatan', $id);
        $this->db->update('sipekerja_kegiatan');
    }
}